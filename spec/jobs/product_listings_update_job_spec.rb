...
RSpec.describe ProductListingsUpdateJob, type: :job, vcr: true do
  ...
  it 'does not perform unless shop application is approved' do
    ...
    expect(Rails.logger).to receive(:error).with("ProductListingsUpdateJob failed: shop '#{shop.shopify_domain}' is not approved by Jublet").once
    ProductListingsUpdateJob.perform_now(shop_domain: shop.shopify_domain, webhook: mock_webhook_data)
  end
end
